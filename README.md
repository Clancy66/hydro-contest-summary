...
1. 可以创建、更新、删除自己的私有总结，创建总结时可以提供模板。
2. 可以查看比赛详情页的公开总结。
3. 如果自己的总结被设置为公开，则不可修改。
...
所有信息存储在全局表 `summary` 中，不会向任何原生数据表添加字段，便于迁移。注意 `problemId` 字段对应题目的 `pid` 字段，路由也是按照 `pid` 进行匹配跳转的，在使用该插件时务必确保比赛中的题目有 `pid` 字段，否则将触发 `undefined` 错误。
